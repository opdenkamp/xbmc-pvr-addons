...
CStdString Dvb::GetHttpXML(CStdString& url)
{
	...
	{
		char buffer[8*1024];
		while (int bytesRead = XBMC->ReadFile(fileHandle, buffer, 8*1024))
...
<status>
	<recordcount>0</recordcount>	Number of running recordings
	<clientcount>0</clientcount>	Number of connected streaming clients
	<epgudate>0</epgudate>			EPG Updater active
	<epgbefore>2</epgbefore>		Default preroll in minutes for timer
	<epgafter>3</epgafter>			Default postroll in minutes for timer
	<recfiles>2282</recfiles>		Number of recorded files
	<timezone>60</timezone>			Timezone offset in minutes	
	<defafterrecord>0</defafterrecord>  Default After Recording action: 0 - Nothing, 1 Shutdown, 2 Standby, 3 Hibernate
	<epglang>deu</epglang>			Default EPG language
	<recfolders></recfolders>		List of the folders with recording files can be disabled by parameter: "norecdirs=1"
</status>
*/
void Dvb::GetTimeZone()  // Should be renamed
{
	CStdString url; 
	url.Format("%s%s", m_strURL.c_str(), "api/status.html?norecdirs=1");  // new param 
	...
	if(xe.error != 0)  {
		...
		CStdString strLNG;
		GetString(xNode, "epglang", strLNG);
		m_strEPGLanguage = strLNG;
	}
}
...
/*<?xml version="1.0" encoding="utf-8" ?><version iver="18350083">DVBViewer Recording Service 1.24.0.6 ([Servername])</version>
iver : (1 shl 24) or (24 shl 16) or (0 shl 8) or (6)
       1.             24.            0.           6
*/
