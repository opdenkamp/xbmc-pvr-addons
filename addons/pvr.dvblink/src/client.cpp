...
bool        g_bGroupRecByTitle      = DEFAULT_GROUPRECBYTITLE;       ///< Group Recordings as Directories by title
...
ADDON_STATUS ADDON_Create(void* hdl, void* props)
{
  ...
  if (!XBMC->GetSetting("group_recordings_by_title", &g_bGroupRecByTitle))
  {
      /* If setting is unknown fallback to defaults */
      XBMC->Log(LOG_ERROR, "Couldn't get 'group_recordings_by_title' setting, falling back to 'true' as default");
      g_bGroupRecByTitle = DEFAULT_GROUPRECBYTITLE;
  }
  ...
  dvblinkclient = new DVBLinkClient(XBMC, PVR, GUI, g_szClientname, g_szHostname, g_lPort, g_bShowInfoMSG, g_szUsername, g_szPassword, g_bAddRecEpisode2title, g_bGroupRecByTitle);
...
ADDON_STATUS ADDON_SetSetting(const char *settingName, const void *settingValue)
{
  ...
  {
      XBMC->Log(LOG_INFO, "Changed Setting 'group_recordings_by_title' from %u to %u", g_bGroupRecByTitle, *(int*)settingValue);
      g_bGroupRecByTitle = *(bool*)settingValue;
      return ADDON_STATUS_NEED_RESTART;
  }
  else if (str == "height")
  {
