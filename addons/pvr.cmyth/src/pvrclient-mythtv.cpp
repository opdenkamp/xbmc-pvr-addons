...
void RecordingRule::SaveTimerString(PVR_TIMER& timer) 
{
  ...
  strncpy(timer.strTitle,m_stringStore.back()->c_str(),sizeof(timer.strTitle));
  ...
  strncpy(timer.strDirectory,m_stringStore.back()->c_str(),sizeof(timer.strDirectory));
  ...
  strncpy(timer.strSummary,m_stringStore.back()->c_str(),sizeof(timer.strSummary));
...
PVR_ERROR PVRClientMythTV::GetRecordings(ADDON_HANDLE handle)
{
  ...
  for (boost::unordered_map< CStdString, MythProgramInfo >::iterator it = m_recordings.begin(); it != m_recordings.end(); it++)
  {
    if(!it->second.IsNull())
    {
      ...
      strncpy(tag.strChannelName,it->second.ChannelName(),sizeof(tag.strChannelName));
      //tag.strPlot=plot;
      strncpy(tag.strPlot,it->second.Description(),sizeof(tag.strPlot));

      CStdString id=it->second.Path();
      //tag.strRecordingId=id;
      strncpy(tag.strRecordingId,it->second.Path(),sizeof(tag.strRecordingId));

      CStdString group=it->second.RecordingGroup();
      //tag.strDirectory=group=="Default"?"":group;
      strncpy(tag.strDirectory,(group=="Default"?"":group),sizeof(tag.strDirectory));
      //tag.strTitle=title;
      strncpy(tag.strTitle,it->second.Title(true),sizeof(tag.strTitle));
      ...
      if((tag.iGenreType==0x10||genre==0x00)&&tag.iDuration>(g_iMinMovieLength*60)&&!regex_series_match&&(it->second.ProgramID().substr(0,2)=="MV"||it->second.ProgramID()==""))
      {
        ...
        strncpy(tag.strDirectory,group,sizeof(tag.strDirectory));
      }
      else
      {
        try {
          ...
          if(boost::regex_search(foldertitle,result,re))
          {
            ...
            strncpy(tag.strTitle,newtitle,sizeof(tag.strTitle));
          }
        }
        catch(...)
        {
      ...
      //strncpy(tag.strIconPath,iconPath.c_str(),sizeof(tag.strIconPath));
      //tag.strFanartPath = fanartPath.c_str();
      //strncpy(tag.strFanartPath,fanartPath.c_str(),sizeof(tag.strFanartPath));

      //Unimplemented
      tag.iLifetime=0;
      tag.iPriority=0;
      //tag.strPlotOutline="";
      strncpy(tag.strPlotOutline,"",sizeof(tag.strPlotOutline));
      //tag.strStreamURL="";
      strncpy(tag.strStreamURL,"",sizeof(tag.strStreamURL));
      ...
      if(dirsep != CStdString::npos)
      {
        ...
        strncpy(tag.strDirectory,"/All Recordings/",sizeof(tag.strDirectory));
        newtitle =  title.substr(dirsep+1,title.size());
        //tag.strTitle = newtitle.c_str();
        strncpy(tag.strTitle,newtitle.c_str(),sizeof(tag.strTitle));
        id.append("@");
        //tag.strRecordingId = id.c_str();
        strncpy(tag.strRecordingId,id.c_str(),sizeof(tag.strRecordingId));
        PVR->TransferRecordingEntry(handle,&tag);
      }
      else
      {
        ...
        strncpy(tag.strTitle,title.c_str(),sizeof(tag.strTitle));
        id.append("@");
        //tag.strRecordingId = id.c_str();
        strncpy(tag.strRecordingId,id.c_str(),sizeof(tag.strRecordingId));
        PVR->TransferRecordingEntry(handle,&tag);
  ...
  for(std::vector <RecordingRule >::iterator it = m_recordingRules.begin();it != m_recordingRules.end(); it++) 
    if(it->Type() == MythTimer::DontRecord || it->Type() == MythTimer::OverrideRecord)
      for(std::vector <RecordingRule >::iterator it2 = m_recordingRules.begin();it2 != m_recordingRules.end(); it2++)
	    ...
	    strncpy(tag.strSummary,summary,sizeof(tag.strSummary));
	    if(g_bExtraDebug)
